...
If @error Then
	_Append("Failed to connect. Exiting...")
	Sleep(1000)
	Exit
EndIf
Global $ConnectedIP = _SocketToIP($ConnectedSocket)
_Append("Connected to " & $ConnectedIP & ":1337") ; Shorter than writing _GUICtrlEdit_AppendText() everywhere.
_Main()
...
Func LLMsg($Message)
	...
	If $Message = "server_shutdown" Then
		_Append(" [INFO] Server shut down.  Disconnected.")
	EndIf
EndFunc
